...
var utils$1 = utils$2.exports = {
  ...
  which: function(name, callback) {
    ...
    which(name, function(err2, result) {
      if (err2) {
        return callback(null, whichCache[name] = "");
      }
    ...
    proto.usingPreset = proto.preset = function(preset) {
      if (typeof preset === "function") {
        ...
        try {
          ...
          throw new Error("preset " + modulePath + " could not be loaded: " + err2.message);
        }
      }
      return this;
    };
    ...
    (function() {
      ...
      async2.each = function(arr, iterator, callback) {
        ...
        _each(arr, function(x) {
          iterator(x, only_once(function(err2) {
            if (err2) {
              callback(err2);
              callback = function() {
              };
      ...
      async2.eachSeries = function(arr, iterator, callback) {
        ...
        var iterate = function() {
          iterator(arr[completed], function(err2) {
            if (err2) {
              callback(err2);
              callback = function() {
              };
        ...
        return function(arr, iterator, callback) {
          ...
          (function replenish() {
            ...
            while (running < limit && started < arr.length) {
              started += 1;
              running += 1;
              iterator(arr[started - 1], function(err2) {
                if (err2) {
                  callback(err2);
                  callback = function() {
                  };
      ...
      var _asyncMap = function(eachfn, arr, iterator, callback) {
        ...
        eachfn(arr, function(x, callback2) {
          iterator(x.value, function(err2, v) {
            results[x.index] = v;
            callback2(err2);
          });
        }, function(err2) {
      ...
      async2.reduce = function(arr, memo, iterator, callback) {
        async2.eachSeries(arr, function(x, callback2) {
          iterator(memo, x, function(err2, v) {
            memo = v;
            callback2(err2);
          });
        }, function(err2) {
      ...
      async2.sortBy = function(arr, iterator, callback) {
        async2.map(arr, function(x, callback2) {
          iterator(x, function(err2, criteria) {
            if (err2) {
              callback2(err2);
            } else {
      ...
      async2.auto = function(tasks, callback) {
        ...
        _each(keys, function(k) {
          ...
          var taskCallback = function(err2) {
      ...
      async2.waterfall = function(tasks, callback) {
        ...
        if (tasks.constructor !== Array) {
          var err2 = new Error("First argument to waterfall must be an array of functions");
          return callback(err2);
        }
        if (!tasks.length) {
          return callback();
      ...
      var _console_fn = function(name) {
        return function(fn) {
          var args = Array.prototype.slice.call(arguments, 1);
          fn.apply(null, args.concat([function(err2) {
          ...
          async2.reduce(
            ...
            function(newargs, fn, cb) {
              fn.apply(that, newargs.concat([function() {
                var err2 = arguments[0];
                var nextargs = Array.prototype.slice.call(arguments, 1);
                cb(err2, nextargs);
              }]));
            },
            function(err2, results) {
              callback.apply(that, [err2].concat(results));
            }
          );
        };
      };
...
function requireProcessor() {
  ...
  function runFfprobe(command) {
    ...
    command.ffprobe(inputProbeIndex, function(err2, data) {
  ...
  processor = function(proto) {
    proto._spawnFfmpeg = function(args, options, processCB, endCB) {
      ...
      this._getFfmpegPath(function(err2, command) {
        if (err2) {
          return endCB(err2);
        ...
        ffmpegProc.on("error", function(err3) {
          endCB(err3);
        });
        var exitError = null;
        function handleExit(err3) {
          if (err3) {
            exitError = err3;
          }
    ...
    proto._prepare = function(callback, readMetadata) {
      ...
      async2.waterfall([
        ...
        function(cb) {
          ...
          self2.ffprobe(0, function(err2, data) {
            if (!err2) {
        ...
        function(cb) {
          ...
          if (flvmeta) {
            self2._getFlvtoolPath(function(err2) {
              cb(err2);
            });
          } else {
            cb();
        ...
        function(args, cb) {
          self2.availableEncoders(function(err2, encoders) {
    ...
    proto.exec = proto.execute = proto.run = function() {
      ...
      var ended = false;
      function emitEnd(err2, stdout, stderr) {
        if (!ended) {
          ended = true;
          if (err2) {
            self2.emit("error", err2, stdout, stderr);
          } else {
            self2.emit("end", stdout, stderr);
          }
        }
      }
      self2._prepare(function(err2, args) {
        if (err2) {
          return emitEnd(err2);
        }
        self2._spawnFfmpeg(
          ...
          function processCB(ffmpegProc, stdoutRing, stderrRing) {
            ...
            if (inputStream) {
              inputStream.source.on("error", function(err3) {
                var reportingErr = new Error("Input stream error: " + err3.message);
                reportingErr.inputStreamError = err3;
            ...
            if (outputStream) {
              ...
              outputStream.target.on("error", function(err3) {
                self2.logger.debug("Output stream error, killing ffmpeg process");
                var reportingErr = new Error("Output stream error: " + err3.message);
                reportingErr.outputStreamError = err3;
          ...
          function endCB(err3, stdoutRing, stderrRing) {
            clearTimeout(self2.processTimer);
            delete self2.ffmpegProc;
            if (err3) {
              if (err3.message.match(/ffmpeg exited with code/)) {
                err3.message += ": " + utils2.extractError(stderrRing.get());
              }
              emitEnd(err3, stdoutRing.get(), stderrRing.get());
            } else {
              var flvmeta = self2._outputs.filter(function(output2) {
                return output2.flags.flvmeta;
              });
              if (flvmeta.length) {
                self2._getFlvtoolPath(function(err4, flvtool) {
                  if (err4) {
                    return emitEnd(err4);
                  }
                  async2.each(
                    flvmeta,
                    function(output2, cb) {
                      spawn(flvtool, ["-U", output2.target], { windowsHide: true }).on("error", function(err5) {
                        cb(new Error("Error running " + flvtool + " on " + output2.target + ": " + err5.message));
    ...
    proto.renice = function(niceness) {
      if (!utils2.isWindows) {
        ...
        if (this.ffmpegProc) {
          ...
          renice.on("error", function(err2) {
            logger.warn("could not renice process " + pid + ": " + err2.message);
          });
...
function requireCapabilities() {
  ...
  capabilities = function(proto) {
    ...
    proto._getFfmpegPath = function(callback) {
      ...
      async2.waterfall([
        ...
        function(ffmpeg2, cb) {
          ...
          utils2.which("ffmpeg", function(err2, ffmpeg22) {
            cb(err2, ffmpeg22);
          });
        }
      ], function(err2, ffmpeg2) {
        if (err2) {
          callback(err2);
        } else {
    ...
    proto._getFfprobePath = function(callback) {
      ...
      async2.waterfall([
        ...
        function(ffprobe2, cb) {
          ...
          utils2.which("ffprobe", function(err2, ffprobe22) {
            cb(err2, ffprobe22);
          });
        },
        ...
        function(ffprobe2, cb) {
          ...
          self2._getFfmpegPath(function(err2, ffmpeg2) {
            if (err2) {
              cb(err2);
    ...
    proto._getFlvtoolPath = function(callback) {
      ...
      async2.waterfall([
        ...
        function(flvtool, cb) {
          ...
          utils2.which("flvmeta", function(err2, flvmeta) {
            cb(err2, flvmeta);
          });
        },
        ...
        function(flvtool, cb) {
          ...
          utils2.which("flvtool2", function(err2, flvtool2) {
            cb(err2, flvtool2);
          });
        }
      ], function(err2, flvtool) {
        if (err2) {
          callback(err2);
        } else {
    ...
    proto.availableFilters = proto.getAvailableFilters = function(callback) {
      ...
      this._spawnFfmpeg(["-filters"], { captureStdout: true, stdoutLines: 0 }, function(err2, stdoutRing) {
        if (err2) {
          return callback(err2);
        }
    ...
    proto.availableCodecs = proto.getAvailableCodecs = function(callback) {
      ...
      this._spawnFfmpeg(["-codecs"], { captureStdout: true, stdoutLines: 0 }, function(err2, stdoutRing) {
        if (err2) {
          return callback(err2);
        }
        var stdout = stdoutRing.get();
    ...
    proto.availableEncoders = proto.getAvailableEncoders = function(callback) {
      ...
      this._spawnFfmpeg(["-encoders"], { captureStdout: true, stdoutLines: 0 }, function(err2, stdoutRing) {
        if (err2) {
          return callback(err2);
        }
        var stdout = stdoutRing.get();
    ...
    proto.availableFormats = proto.getAvailableFormats = function(callback) {
      ...
      this._spawnFfmpeg(["-formats"], { captureStdout: true, stdoutLines: 0 }, function(err2, stdoutRing) {
        if (err2) {
          return callback(err2);
        }
        var stdout = stdoutRing.get();
...
function requireFfprobe() {
  ...
  ffprobe = function(proto) {
    proto.ffprobe = function() {
      ...
      var ended = false;
      function handleCallback(err2, data) {
        if (!ended) {
          ended = true;
          callback(err2, data);
        }
      ...
      this._getFfprobePath(function(err2, path2) {
        if (err2) {
          return handleCallback(err2);
        } else if (!path2) {
        ...
        if (input.isStream) {
          ffprobe2.stdin.on("error", function(err3) {
            if (["ECONNRESET", "EPIPE", "EOF"].indexOf(err3.code) >= 0) {
              return;
            }
            handleCallback(err3);
          });
...
function requireRecipes() {
  ...
  recipes = function recipes2(proto) {
    ...
    proto.takeScreenshots = proto.thumbnail = proto.thumbnails = proto.screenshot = proto.screenshots = function(config, folder) {
      ...
      function getMetadata(cb) {
        if (metadata2) {
          ...
          self2.ffprobe(function(err2, meta) {
            metadata2 = meta;
            cb(err2, meta);
          });
        }
      ...
      async2.waterfall([
        ...
        function computeTimemarks(next) {
          if (config.timemarks.some(function(t) {
            ...
            getMetadata(function(err2, meta) {
              if (err2) {
                next(err2);
              } else {
        ...
        function createDirectory(filenames, next) {
          fs.exists(config.folder, function(exists) {
            if (!exists) {
              fs.mkdir(config.folder, function(err2) {
                if (err2) {
                  next(err2);
                } else {
                  next(null, filenames);
      ...
      ], function runCommand(err2, filenames) {
        if (err2) {
    ...
    proto.mergeToFile = proto.concatenate = proto.concat = function(target, options) {
      ...
      var self2 = this;
      this.ffprobe(this._inputs.indexOf(fileInput), function(err2, data) {
        if (err2) {
          return self2.emit("error", err2);
        }
...
const dragIconPath = require$$1.join(require$$1.dirname(require$$1.dirname(require$$1.dirname(require$$1.dirname(__filename$1)))), "64x64.png");
...
const thumbnailsPath = require$$1.join(electron.app.getPath("userData"), "thumbnails");
console.log("Config:", { clipsPath, thumbnailsPath, userData: electron.app.getPath("userData") });
...
async function createWindow() {
  ...
  mainWindow = new electron.BrowserWindow({
    ...
    // Show when ready
    webPreferences: {
      preload: require$$1.join(__dirname$1, "../preload/index.js"),
  ...
  try {
    if (isDev) {
      ...
      await mainWindow.loadFile(require$$1.join(__dirname$1, "../renderer/index.html"));
...
electron.ipcMain.handle("clips:getList", async () => {
  try {
    if (!require$$0.existsSync(clipsPath)) {
      await promises.mkdir(clipsPath, { recursive: true });
      return [];
    }
    const files = await promises.readdir(clipsPath);
    const clips = await Promise.all(
      files.filter((file) => file.endsWith(".mp4")).map(async (filename) => {
        const filePath = require$$1.join(clipsPath, filename);
        const stats = await promises.stat(filePath);
        const metadataPath = filePath.replace(".mp4", ".clipvault.json");
        let metadata2 = null;
        try {
          if (require$$0.existsSync(metadataPath)) {
            const content = await promises.readFile(metadataPath, "utf-8");
            metadata2 = JSON.parse(content);
          }
        } catch (e) {
...
electron.ipcMain.handle("clips:saveMetadata", async (_2, clipId, metadata2) => {
  try {
    const metadataPath = require$$1.join(clipsPath, `${clipId}.clipvault.json`);
    await promises.writeFile(metadataPath, JSON.stringify(metadata2, null, 2), "utf-8");
    return true;
  } catch (error) {
...
electron.ipcMain.handle("clips:getMetadata", async (_2, clipId) => {
  try {
    const metadataPath = require$$1.join(clipsPath, `${clipId}.clipvault.json`);
    if (!require$$0.existsSync(metadataPath)) {
      return null;
...
electron.ipcMain.handle("system:openFolder", async () => {
  await electron.shell.openPath(clipsPath);
});
electron.ipcMain.handle("dialog:save", async (_2, options) => {
  if (!mainWindow) return null;
  const result = await electron.dialog.showSaveDialog(mainWindow, options);
  return result;
});
electron.ipcMain.handle("clips:generateThumbnail", async (_2, clipId, videoPath2) => {
  try {
    if (!require$$0.existsSync(thumbnailsPath)) {
      await promises.mkdir(thumbnailsPath, { recursive: true });
    }
    const thumbnailFilename = `${clipId}.jpg`;
    const thumbnailPath = require$$1.join(thumbnailsPath, thumbnailFilename);
    if (require$$0.existsSync(thumbnailPath)) {
      return `clipvault://thumb/${encodeURIComponent(thumbnailFilename)}`;
    }
    return new Promise((resolve2, reject2) => {
      ffmpeg(videoPath2).screenshots({
        timestamps: ["10%"],
        // Take screenshot at 10% into the video
        filename: thumbnailFilename,
        folder: thumbnailsPath,
        size: "480x270"
        // 16:9 aspect ratio thumbnail
      }).on("end", () => {
        resolve2(`clipvault://thumb/${encodeURIComponent(thumbnailFilename)}`);
      }).on("error", (err2) => {
...
electron.ipcMain.handle("video:getFileUrl", async (_2, filename) => {
  try {
    const filePath = require$$1.join(clipsPath, filename);
...
electron.ipcMain.handle("clips:getVideoMetadata", async (_, videoPath) => {
  try {
    return new Promise((resolve, reject) => {
      ffmpeg.ffprobe(videoPath, (err, metadata) => {
        ...
        resolve({
          ...
          videoCodec: (videoStream == null ? void 0 : videoStream.codec_name) || "",
...
let exportPreviewWindow = null;
function createExportPreviewWindow(filePath) {
  if (exportPreviewWindow && !exportPreviewWindow.isDestroyed()) {
    exportPreviewWindow.close();
  }
  exportPreviewWindow = new electron.BrowserWindow({
    width: 600,
    height: 500,
    alwaysOnTop: true,
    modal: false,
  ...
  exportPreviewWindow.webContents.on("crashed", () => {
    ...
    exportPreviewWindow == null ? void 0 : exportPreviewWindow.close();
...
<body>
  ...
  exportPreviewWindow.webContents.on("ipc-message", (event, channel, data) => {
    if (channel === "export:startDrag") {
      ...
      try {
        let dragIcon;
        if (require$$0.existsSync(dragIconPath)) {
          dragIcon = electron.nativeImage.createFromPath(dragIconPath);
          console.log("Drag icon loaded from:", dragIconPath);
        } else {
          console.warn("Drag icon not found at:", dragIconPath);
        }
        exportPreviewWindow == null ? void 0 : exportPreviewWindow.webContents.startDrag({
          file: data,
          icon: dragIcon || electron.nativeImage.createEmpty()
        });
      } catch (error) {
        console.error("Error starting drag:", error);
      }
    } else if (channel === "export:openFolder") {
      const folderPath = require$$1.dirname(data);
      electron.shell.openPath(folderPath);
    }
  });
}
electron.ipcMain.handle("export:showPreview", async (_2, filePath) => {
  createExportPreviewWindow(filePath);
  return { success: true };
});
electron.ipcMain.handle("audio:extractTracks", async (_2, clipId, videoPath2) => {
  try {
    const audioCachePath = require$$1.join(thumbnailsPath, "audio");
    if (!require$$0.existsSync(audioCachePath)) {
      await promises.mkdir(audioCachePath, { recursive: true });
    }
    const track1Path = require$$1.join(audioCachePath, `${clipId}_track1.m4a`);
    const track2Path = require$$1.join(audioCachePath, `${clipId}_track2.m4a`);
    const results = {};
    const track1Exists = require$$0.existsSync(track1Path);
    const track2Exists = require$$0.existsSync(track2Path);
    if (track1Exists) {
    ...
    const metadata2 = await new Promise((resolve2, reject2) => {
      ffmpeg.ffprobe(videoPath2, (err2, data) => {
        if (err2) reject2(err2);
        else resolve2(data);
      });
    });
    const audioStreams2 = metadata2.streams.filter((s) => s.codec_type === "audio");
    if (!track1Exists && audioStreams2.length >= 1) {
      await new Promise((resolve2, reject2) => {
        ffmpeg(videoPath2).outputOptions(["-map 0:a:0", "-c:a aac", "-b:a 128k"]).save(track1Path).on("end", () => resolve2()).on("error", (err2) => reject2(err2));
      });
      results.track1 = `clipvault://audio/${encodeURIComponent(`${clipId}_track1.m4a`)}`;
    }
    if (!track2Exists && audioStreams2.length >= 2) {
      await new Promise((resolve2, reject2) => {
        ffmpeg(videoPath2).outputOptions(["-map 0:a:1", "-c:a aac", "-b:a 128k"]).save(track2Path).on("end", () => resolve2()).on("error", (err2) => reject2(err2));
      });
      results.track2 = `clipvault://audio/${encodeURIComponent(`${clipId}_track2.m4a`)}`;
    }
    return results;
...
electron.ipcMain.handle(
  ...
  async (_2, params) => {
    try {
      ...
      const vol1 = audioTrack1Volume ?? 1;
      const vol2 = audioTrack2Volume ?? 1;
      const exportedClipsPath = require$$1.join(clipsPath, "exported-clips");
      if (!require$$0.existsSync(exportedClipsPath)) {
        await promises.mkdir(exportedClipsPath, { recursive: true });
      }
      const outputPath = require$$1.join(exportedClipsPath, exportFilename);
      return new Promise((resolve2, reject2) => {
        const command = ffmpeg(clipPath).seekInput(trimStart).duration(duration).videoCodec("copy");
...
electron.app.whenReady().then(() => {
  ...
  createWindow().catch((err2) => {
    console.error("Failed to create window:", err2);
  });
  electron.protocol.registerFileProtocol("clipvault", (request, callback) => {
    try {
      ...
      const type = host;
      const encodedFilename = pathname.substring(1);
      ...
      if (type === "clip") {
        ...
        basePath = require$$1.join(thumbnailsPath, "audio");
      } else if (type === "exported") {
        basePath = require$$1.join(clipsPath, "exported-clips");
      } else {
        console.error("Unknown resource type:", type);
        callback({ error: -2 });
        return;
      }
      const filePath = require$$1.join(basePath, filename);
